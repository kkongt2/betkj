...
race dates. Equal 1,000 KRW per selected horse; no compounding/chasing.
"""
...
from profit_model import export
from place_audit import robustness, influence
...
EXPERIMENT=dict(train=['20220101','20241231'],calibrate=['20250101','20250331'],
    ...
    dividend_losses=['gamma','poisson'],min_edges=[0,.05,.1,.2,.35,.5,.75,1.0],max_per_race=[1,2,3],min_dividends=[1,2,3],
    min_selection_bets=200,min_selection_dates=60,unit_stake_krw=1000,
    objective='maximum total settled pre-tax net profit at equal stake per horse',
    selection_rule='maximize total net among policies positive in BOTH Apr-Jul and Aug-Dec temporal folds, each >=50 bets and positive after best-day removal; if none, retain best supported research candidate and abstain',
    deployment_gate='selection temporal stability; evaluation profit positive after 10% payout stress and after removing top 5 returns; selection profit > 0; evaluation >=200 bets and >=60 dates; date-bootstrap 95% ROI lower bound >0; profit remains >0 without best day')

def choose(races,policy):
    out=[]
    for r in races:
        eligible=[c for c in r['candidates'] if c['edge']>=policy['min_edge'] and c['dividend']>=policy['min_dividend']]
...
def run():
    ...
    xx=[];yy=[];segs=[];offset=0
    for r in use:
        X=np.asarray(r['X']);nums=[[n] for n in r['numbers']]
    ...
    for loss in EXPERIMENT['dividend_losses']:
        ...
        for s in segs:
            ...
            candidates=[dict(field_size=s['hi']-s['lo'],numbers=ns,prob=float(p[i]),dividend=float(d[i]),edge=float(p[i]*d[i]-1),gross=float(gross[i])) for i,ns in zip(range(s['lo'],s['hi']),s['numbers'])]
        ...
        for edge in EXPERIMENT['min_edges']:
            for k,dmin in [(k,d) for k in EXPERIMENT['max_per_race'] for d in EXPERIMENT['min_dividends']]:
                policy=dict(dividend_loss=loss,min_edge=edge,max_per_race=k,min_dividend=dmin)
                s=stats(choose(selection,policy));eligible=s['bets']>=200 and s['dates']>=60
                halves=[{k:v for k,v in stats(choose([r for r in selection if (r['date']<'20250801')==early],policy)).items() if k not in ('curve','months')} for early in [True,False]]
                stable=eligible and all(h['profit_krw']>0 and h['bets']>=50 for h in halves) and s['without_best_day_profit_krw']>0
                options.append(dict(policy=policy,eligible=eligible,stable=stable,temporal_folds=halves,selection={k:v for k,v in s.items() if k not in ('curve','months')}))
    ...
    stable=[o for o in eligible if o['stable']]
    chosen=max(stable or eligible,key=lambda o:(o['selection']['profit_units'],-o['selection']['bets']))
    ...
    robust=robustness(bets,stats)
    robust['conservative_profit_krw']=min(round(evaluation['roi_95ci'][0]*evaluation['stake_krw']),robust['stress_10pct_profit_krw'],robust['without_top5_returns_profit_krw'],evaluation['without_best_day_profit_krw']) if evaluation['bets'] else 0
    auditmask=dates>='20260101'
    calibration_audit=dict(brier=float(brier_score_loss(y[auditmask],p[auditmask])),log_loss=float(log_loss(y[auditmask],p[auditmask])),bins=[dict(predicted=float(p[auditmask][(p[auditmask]>=lo)&(p[auditmask]<lo+.1)].mean()),observed=float(y[auditmask][(p[auditmask]>=lo)&(p[auditmask]<lo+.1)].mean()),count=int(((p[auditmask]>=lo)&(p[auditmask]<lo+.1)).sum())) for lo in np.arange(0,1,.1) if ((p[auditmask]>=lo)&(p[auditmask]<lo+.1)).any()])
    baseline=[]
    for r in test:
        v=max(r['candidates'],key=lambda c:c['prob']);baseline.append(dict(v,date=r['date'],venue=r['venue'],race_no=r['race_no']))
    approved=bool(chosen['stable'] and robust['stress_10pct_profit_krw']>0 and robust['without_top5_returns_profit_krw']>0 and selection['profit_units']>0 and evaluation['bets']>=200 and evaluation['dates']>=60 and evaluation['roi_95ci'][0]>0 and evaluation['without_best_day_profit_krw']>0)
    report=dict(schema=1,name='PLACE flat-stake profit v1',market='place',calibration_audit=calibration_audit,robustness=robust,selection_temporal_folds=chosen['temporal_folds'],selection_stable=chosen['stable'],objective=EXPERIMENT['objective'],unit_stake_krw=1000,policy=policy,
        ...
        counts=dict(history_races=len(rows),payout_races=len(payout_rows),used_races=len(use),horses=len(y),
        ...
        'Two models and 144 policies are a bounded search, not proof of the globally most profitable strategy.',
        'Daily drawdown ignores intraday ordering. Multiple horses in a race are correlated.'])
    model=dict(schema=1,feature_version=VERSION,features=FEATURES,market='place',
    ...
    importance=influence(classifier,models[loss],X[cal],y[cal],gross[cal],FEATURES,a,b,scales[loss])
    Path('data/weights.json').write_text(json.dumps(importance,ensure_ascii=False,separators=(',',':')))
    fixture=next(r for r in use if r['date']>='20260101');FX=np.asarray(fixture['X'])
    fp=expit(a*logit(classifier.predict_proba(FX)[:,1])+b);fd=np.maximum(1,models[loss].predict(FX)*scales[loss])
    Path('data/parity.json').write_text(json.dumps(dict(horses=[dict(number=n,features_v7=x) for n,x in zip(fixture['numbers'],fixture['X'])],expected=[dict(numbers=[n],prob=float(pr),dividend=float(di)) for n,pr,di in zip(fixture['numbers'],fp,fd)]),separators=(',',':')))
