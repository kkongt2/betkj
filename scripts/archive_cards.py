    ...
    for r in raw:
        ...
        if payout:
            card['official_result']=dict(status='confirmed',version=2,source=payout['source'],checked_at=now.isoformat(timespec='seconds'),
                ...
                place=dict(status='confirmed',payouts=[dict(numbers=list(map(int,k.split('-'))),odds=v) for k,v in payout['payouts'].items()]))
        cards[key(r)]=card
    for k,card in cards.items():
        payout=pp.get(k)
        if payout and card['date']<cutoff:
            result=card.setdefault('official_result',{})
            result.update(status='confirmed',version=2,source=payout['source'])
            result['place']=dict(status='confirmed',payouts=[dict(numbers=[int(n)],odds=d) for n,d in payout['payouts'].items()])
    ...
    if fetch_missing:
        ...
        for report in manifest['reports']:
            ...
            for rn,_ in report.get('skips',[]):
                ...
                if existing and existing.get('official_result',{}).get('place',{}).get('status') in ('confirmed','refund'):continue
                try:
                    card=existing
