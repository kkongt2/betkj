...
for(const b of bets){assert.ok(b.date>='20260101'&&b.date<='20260910');const p=index.get(`${b.date}/${b.venue}/${b.race_no}`);assert.ok(p);assert.equal(b.gross,p[b.numbers.join('-')]||0);assert.ok(b.edge>=model.policy.min_edge);assert.ok(b.dividend>=model.policy.min_dividend);assert.equal(b.numbers.length,1);profit+=(b.gross-1)*1000;}
...
assert.equal(model.approved,report.selection_stable&&report.robustness.stress_10pct_profit_krw>0&&report.robustness.without_top5_returns_profit_krw>0&&s.profit_units>0&&e.bets>=200&&e.dates>=60&&e.roi_95ci[0]>0&&e.without_best_day_profit_krw>0);
...
assert.deepEqual(api.predict({...race,official_result:{place:{payouts:[{numbers:[1],odds:999}]}}},model),actual);
