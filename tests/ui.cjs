...
const context=vm.createContext({document:{getElementById:get,createElement:()=>new Element()},Date:FixedDate,Intl,console,setInterval:()=>{},PLACE:require('../model.js'),fetch:async path=>({ok:true,json:async()=>JSON.parse(fs.readFileSync(path.split('?')[0]==='data/latest.json'?'tests/ui-fixture.json':path.split('?')[0],'utf8'))})});
...
setImmediate(()=>{
  ...
  get('dates').children[0].click();assert.match(get('race-content').innerHTML,/연승 실제 결과/);assert.match(get('race-content').innerHTML,/1\.5배/);assert.match(get('race-content').innerHTML,/추정 적중확률/);assert.match(get('race-content').innerHTML,/손익분기 배당/);assert.match(get('race-content').innerHTML,/과거 데이터 재계산/);
  ...
  assert.match(get('race-content').innerHTML,/출발 시각 경과/);assert.match(get('race-content').innerHTML,/공식 연승 결과가 아직/);assert.match(get('race-content').innerHTML,/손익분기 배당/);
  assert.match(get('metrics').innerHTML,/5,800원/);assert.match(get('robust').innerHTML,/최고 수익일/);
...
setImmediate(()=>{assert.match(get('weights').innerHTML,/확률 모델 영향 비중/);assert.doesNotMatch(get('race-content').innerHTML,/undefined/);});
